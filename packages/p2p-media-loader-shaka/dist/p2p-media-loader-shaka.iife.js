...
this.p2pml.shaka = (function(exports) {
	...
	*/
	...
	function getAudioStreamProperties(variant, audio, isMain) {
		var _ref, _audio$label;
		const name = (_ref = (_audio$label = audio.label) !== null && _audio$label !== void 0 ? _audio$label : audio.originalId) !== null && _ref !== void 0 ? _ref : void 0;
		return {
	...
	var ManifestParserDecorator = class {
		constructor(shaka, originalManifestParser) {
			_defineProperty(this, "shaka", void 0);
			_defineProperty(this, "originalManifestParser", void 0);
			_defineProperty(this, "debug", (0, import_browser.debug)("p2pml-shaka:manifest-parser"));
			_defineProperty(this, "isHls", void 0);
		...
		start(uri, playerInterface) {
			...
			return _asyncToGenerator(function* () {
				...
				_this.setP2PMediaLoaderData(p2pml);
				const manifest = yield _this.originalManifestParser.start(uri, playerInterface);
				if (!p2pml) return manifest;
				if (_this.isHls) _this.hookHlsStreamMediaSequenceTimeMaps(manifest.variants);
				_this.processStreams(manifest.variants);
				return manifest;
			})();
		}
		stop() {
		...
		hookSegmentIndex(stream) {
			...
			const substituteSegmentIndexGet = (segmentIndex, callFromCreateSegmentIndexMethod = false) => {
				...
				const customGet = (position) => {
					...
					if (reference === prevReference || !((_this$player = this.player) === null || _this$player === void 0 ? void 0 : _this$player.isLive()) && stream.isSegmentIndexAlreadyRead) return reference;
					...
					try {
						...
						var _this$player2;
						if (!stream.isSegmentIndexAlreadyRead || !!((_this$player2 = this.player) === null || _this$player2 === void 0 ? void 0 : _this$player2.isLive()) || !callFromCreateSegmentIndexMethod) segmentIndex.get = customGet;
					}
					return reference;
				};
			...
			stream.createSegmentIndex = _asyncToGenerator(function* () {
		...
		hookHlsStreamMediaSequenceTimeMaps(variants) {
			...
			const manifestVariantsMap = maps.find((map) => {
				...
				return typeof item === "object" && (item === null || item === void 0 || (_item$streams = item.streams) === null || _item$streams === void 0 ? void 0 : _item$streams.createSegmentIndex);
			});
			...
			for (const variant of manifestVariantMapValues) {
				var _variant$stream;
				if (variant === null || variant === void 0 || (_variant$stream = variant.stream) === null || _variant$stream === void 0 ? void 0 : _variant$stream.mediaSequenceTimeMap) continue;
				const mediaSequenceTimeMap = getMapPropertiesFromObject(variant).find((map) => {
					var _map$entries$next$val;
					const [key, value] = (_map$entries$next$val = map.entries().next().value) !== null && _map$entries$next$val !== void 0 ? _map$entries$next$val : [];
	...
	function getByteRangeFromHeaderString(rangeStr) {
		if (!(rangeStr === null || rangeStr === void 0 ? void 0 : rangeStr.includes("bytes="))) return void 0;
	...
	function getSegmentInfoFromReference(segmentReference) {
		var _uris$, _segmentReference$get;
		const uris = segmentReference.getUris();
		const responseUrl = (_uris$ = uris[1]) !== null && _uris$ !== void 0 ? _uris$ : uris[0];
		const start = segmentReference.getStartByte();
		const end = (_segmentReference$get = segmentReference.getEndByte()) !== null && _segmentReference$get !== void 0 ? _segmentReference$get : void 0;
	...
	var SegmentManager = class {
		constructor(streamInfo, core) {
			...
			_defineProperty(this, "streamInfo", void 0);
			this.core = core;
	...
	var Loader = class {
		constructor(shaka, core, streamInfo) {
			...
			_defineProperty(this, "core", void 0);
			_defineProperty(this, "streamInfo", void 0);
			_defineProperty(this, "loadArgs", void 0);
			this.shaka = shaka;
			this.core = core;
			this.streamInfo = streamInfo;
		}
		...
		handleManifestLoading(loadingPromise) {
			var _this = this;
			return _asyncToGenerator(function* () {
				if (!_this.streamInfo.manifestResponseUrl) {
					const response = yield loadingPromise;
					_this.setManifestResponseUrl(response.uri);
				}
			})();
		}
		loadSegment(segmentUrl, originalRequest) {
			...
			const loadSegment = function() {
				var _ref = _asyncToGenerator(function* () {
					...
					_this2.core.loadSegment(segmentRuntimeId, callbacks);
					try {
						...
						if (error instanceof CoreRequestError) {
							const { Error: ShakaError } = _this2.shaka.util;
							if (error.type === "aborted") throw new ShakaError(ShakaError.Severity.RECOVERABLE, ShakaError.Category.NETWORK, _this2.shaka.util.Error.Code.OPERATION_ABORTED);
						}
						throw error;
					}
				});
	...
	*/
	var ShakaP2PEngine = class ShakaP2PEngine {
		...
		*/
		constructor(config, shaka = window.shaka) {
			_defineProperty(this, "player", void 0);
			_defineProperty(this, "shaka", void 0);
			_defineProperty(this, "streamInfo", {});
			_defineProperty(this, "core", void 0);
			_defineProperty(this, "segmentManager", void 0);
			_defineProperty(this, "requestFilter", void 0);
			_defineProperty(this, "updatePlayerEventHandlers", (type) => {
				const { player } = this;
				if (!player) return;
			...
			_defineProperty(this, "onVariantChanged", () => {
			...
			_defineProperty(this, "handlePlayerLoaded", () => {
				if (!this.player) return;
				this.core.setIsLive(this.player.isLive());
				this.updateMediaElementEventHandlers("register");
			});
			_defineProperty(this, "handlePlayerUnloading", () => {
				this.destroyCurrentStreamContext();
				this.updateMediaElementEventHandlers("unregister");
			});
			_defineProperty(this, "destroyCurrentStreamContext", () => {
			...
			_defineProperty(this, "updateMediaElementEventHandlers", (type) => {
				var _this$player;
				const media = (_this$player = this.player) === null || _this$player === void 0 ? void 0 : _this$player.getMediaElement();
				if (!media) return;
				const method = type === "register" ? "addEventListener" : "removeEventListener";
	...
	//#endregion
	exports.Core = Core;
	exports.ShakaP2PEngine = ShakaP2PEngine;
	return exports;
})({});

//# sourceMappingURL=p2p-media-loader-shaka.iife.js.map